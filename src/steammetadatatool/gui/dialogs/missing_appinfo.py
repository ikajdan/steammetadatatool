...
def _select_appinfo_file(parent: QWidget, initial_dir: Path) -> Path | None:
    selected_path, _selected_filter = QFileDialog.getOpenFileName(
        parent,
        _("Select appinfo.vdf"),
        str(_existing_parent_dir(initial_dir)),
        _("Steam appinfo (appinfo.vdf);;VDF files (*.vdf);;All files (*)"),
    )
...
class MissingAppInfoDialog(QDialog):
    def __init__(
        ...
        heading = QLabel(_("Steam Metadata File not Found"), self)
        ...
        next_step = QLabel(
            _("Choose a correct appinfo.vdf file to continue."),
            self,
        )
        ...
        select_button = QPushButton(_("Select File…"), self)
        ...
        quit_button = QPushButton(_("Quit"), self)
